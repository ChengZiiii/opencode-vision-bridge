...
- [x] 1.1 `package.json`: delete `scripts.postinstall`; replace `prebuild` +
...
- [x] 1.2 Delete `scripts/install-skill.mjs`. Verify: `scripts/` directory no
  longer exists in the repo.

## 2. Docs and comments

- [x] 2.1 `plugin.ts`: update the comments that reference the npm postinstall
  ...
  returns nothing.
- [x] 2.2 `README.md`: drop postinstall wording; document the official
...
- [x] 2.3 `AGENTS.md`: update dev rules — build is `bun run bundle`;
